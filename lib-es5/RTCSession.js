  ...
  _createClass(RTCSession, [{
    ...
    value: function _createRTCConnection(pcConfig, rtcConstraints) {
      ...
      this._connection.addEventListener('iceconnectionstatechange', function () {
        ...
        // 当hold时，15s后sip服务认为rtp无媒体流则断开连接，触发ICE连接状态变更为dicconnected， 若ICE在之后尝试重连失败则更新状态为failed。
        // 为避免服务端的在这种场景下结束会话，基于七鱼呼入呼出的media.direction特性进行处理

        if (state === 'failed' && !_this12._remoteHold) {
          _this12.terminate({
